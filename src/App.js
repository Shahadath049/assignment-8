...
A prop can be passed on to the child in the state of a parent. They apply to the same value but can only be updated by the parent variable.A prop can be passed on to the child in the state of a parent. They apply to the same value but can only be updated by the parent variable.A state change takes place on the basis of the user input, which triggers an occurrence. React  components are often made on the basis of state data. The initial knowledge is held by the State.
